...
public:
    ...
    TConvexHull(std::string const &sphere_proto, SphereTag);

    TConvexHull(std::string const &cylinder_proto, CylinderTag);

    TConvexHull(std::string const &prism_proto, PrismTag);

    TConvexHull(std::string const &filename, FileTag);
    ...
    std::vector<TTriangle> _triangles;
