...
inline std::string trim(const std::string &str) {
    auto front = std::find_if_not(
        str.begin(), str.end(), [](int c) { return std::isspace(c); });
    ...
    try {
        ...
        if (length != string.length()) {
            throw chemfiles::Error(
                "Can not convert '" + string + "' to number");
        }
        return value;
        ...
        throw chemfiles::Error(
            "'" + string + "' is out of range for double type");
    }
}
...
inline long long int string2longlong(const std::string &string) {
    try {
        ...
        throw chemfiles::Error(
            "'" + string + "' is out of range for long long int type");
    }
}
