...
Cavity::Cavity(Cavity const &hueco, std::vector<double> const &vector,
    ...
    move_cells(hueco._inner_cells, hueco._inner_info, vector, step_size);
    move_cells(hueco._outer_cells, hueco._outer_info, vector, step_size);
    return;
}

void Cavity::move_cells(std::vector<Tetrahedron> const &cells,
    std::vector<TetraInfo> const &info, std::vector<double> const &vector,
    double const step_size) {
