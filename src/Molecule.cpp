...
SoAMolecule::SoAMolecule(std::string const &filename, bool const atom_only) {
    ...
    for (auto const &residuo : input_pdb_top.residues()) {
        ...
        for (auto const &i : residuo) {
            ...
            _info.push(i, vdw, residuo.id().value(), res_name);
            _data.push(in_xyz[i][0], in_xyz[i][1], in_xyz[i][2]);
